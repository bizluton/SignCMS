-- Make get_channel_schedule_intervals bucket calendar blocks by *local* days
-- in the supplied IANA timezone (instead of UTC days), so a 4/24 18:55 Taipei
-- start renders at minute 1135 on 4/24 rather than minute 655 on 4/24 UTC.
...
BEGIN
  ...
  RETURN QUERY
  WITH cal AS (
    -- Walk each *local* day (in v_tz) overlapped by [start_at, end_at).
    -- d_local is a naive timestamp representing midnight of that local day;
    -- (d_local AT TIME ZONE v_tz) converts it back to a UTC timestamptz boundary.
    SELECT
      b.id AS block_id,
      b.design_project_id,
      b.name,
      ...
      GREATEST(
        0,
        EXTRACT(EPOCH FROM (
          GREATEST(b.start_at, (d_local AT TIME ZONE v_tz))
          - (d_local AT TIME ZONE v_tz)
        )) / 60
      )::int AS start_min,
      LEAST(
        24 * 60,
        EXTRACT(EPOCH FROM (
          LEAST(b.end_at, ((d_local + interval '1 day') AT TIME ZONE v_tz))
          - (d_local AT TIME ZONE v_tz)
        )) / 60
      )::int AS end_min
    ...
    CROSS JOIN LATERAL generate_series(
      GREATEST(
        date_trunc('day', (b.start_at AT TIME ZONE v_tz)),
        date_trunc('day', (_from AT TIME ZONE v_tz))
      ),
      LEAST(
        ((b.end_at AT TIME ZONE v_tz) - interval '1 microsecond'),
        ((_to AT TIME ZONE v_tz) - interval '1 microsecond')
      ),
      interval '1 day'
    ) AS d_local
  ...
  weekly AS (
    -- Weekly blocks: walk each local day in window; keep matching weekday in v_tz.
    SELECT
      b.id AS block_id,
      b.design_project_id,
      b.name,
    ...
    FROM public.channel_blocks b
    CROSS JOIN LATERAL generate_series(
      (_from AT TIME ZONE v_tz)::date,
      ((_to AT TIME ZONE v_tz) - interval '1 microsecond')::date,
      interval '1 day'
    ) AS d
    WHERE b.channel_id = _channel_id
      AND b.enabled = true
  ...
  SELECT * FROM cal WHERE end_min > start_min
  UNION ALL
  SELECT * FROM weekly WHERE end_min > start_min
