...
-- 1. Update handle_new_user to set all permissions on Default team creation
...
BEGIN
  ...
  -- 1) Try explicit cs_agent metadata first
  ...
  -- 2) Fallback: match by email even without cs_agent metadata
  ...
  IF _invite_token IS NOT NULL AND _invite_token != '' THEN
    ...
    IF _invite_org_id IS NOT NULL THEN
      ...
      IF _team_id IS NULL THEN
        ...
        VALUES (_invite_org_id, 'Default', 'Default team', NEW.id, _default_perms)
  ...
  IF _org_name IS NOT NULL AND _org_name != '' THEN
    ...
    IF _team_id IS NULL THEN
      INSERT INTO public.teams (org_id, name, description, created_by, permissions)
      VALUES (_org_id, 'Default', 'Default team', NEW.id, _default_perms) RETURNING id INTO _team_id;
    END IF;
...
-- 2. Update existing Default teams with empty permissions
UPDATE public.teams
SET permissions = '["screens","media","schedules","publish","studio"]'::jsonb
WHERE name = 'Default' AND (permissions = '[]'::jsonb OR permissions IS NULL);
