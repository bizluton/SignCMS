...
BEGIN
  ...
  IF _invite_token IS NOT NULL AND _invite_token != '' THEN
    -- Look up valid pending invitation matching this email
    ...
    IF _invite_org_id IS NOT NULL THEN
      ...
      -- Get or create default team in the invited org
      ...
      RETURN NEW;
    END IF;
    -- If invitation is invalid/expired, fall through to org_name logic
  END IF;

  -- Handle organization from signup metadata (original flow)
